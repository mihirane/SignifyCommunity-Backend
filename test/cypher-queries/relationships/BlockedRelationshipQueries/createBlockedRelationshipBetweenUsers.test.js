...
const getJSONFromCsv = require("../../../../test-config/get-json-from-csv/getJSONFromCsv.js")("test-config/dummy-data-csv-files/cypher-queries/relationships/BlockedRelationshipQueries/createBlockedRelationshipBetweenUsers");
const createTestObject = require("../../../../test-config/create-test-object/createTestObject.js");
const LoadDummyDataToNeo4j = require("./../../../../test-config/load-dumy-data-to-neo4j/loadDummyDataToNeo4j.js");
const csvFilesPath = "cypher-queries/relationships/BlockedRelationshipQueries/createBlockedRelationshipBetweenUsers";

const loadDummyDataToNeo4j = new LoadDummyDataToNeo4j(driver, csvFilesPath);

describe("Testing create blocked relationship between users query", () => {
    ...
    it("create blocked relationship between users query", () => {
            ...
            .then(async ({
                ...
                for (let i = await 0; i <= dummyUsersData.length - 1; i++) {
                    for (let j = await (i + 1); j <= dummyUsersData.length - 1; j++) {
                        await executeQuery(blockedRelationshipQueries.createBlockedRelationshipBetweenUsers.query, blockedRelationshipQueries.createBlockedRelationshipBetweenUsers.params(dummyUsersData[i].recentFacebookAccessToken, dummyUsersData[j].id))
                            .then(async (records) => {
